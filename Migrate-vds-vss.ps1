...
# --- [ HELPER FUNCTION: VALIDATE NIC NAME FORMAT & AVAILABILITY ] ---
function Read-NicName {
    param ([array]$ValidNics)
    $nicName = ""
    do {
        $nicName = (Read-Host "Enter the exact name of the vmnic to remove (e.g., vmnic1)").Trim().ToLower()
        if ($nicName -notin $ValidNics) {
            Write-Host "  [!] Invalid input. '$nicName' is not attached to this vDS." -ForegroundColor Red
            Write-Host "      Valid options for this switch: $($ValidNics -join ', ')" -ForegroundColor Yellow
        }
    } while ($nicName -notin $ValidNics)
    
    Write-Log "  [USER INPUT] Selected NIC: '$nicName'" "DarkGray"
    return $nicName
}
...
if ($vcVm) {
    ...
    foreach ($netMoRef in $vcVm.Network) {
        ...
        # Determine if it's on a Distributed Switch
        ...
        # Determine if it's on a Standard Switch
        elseif ($netView -is [VMware.Vim.Network] -or $netView.GetType().Name -eq 'Network') {
            ...
            if ($targetPg) {
                ...
                if ($vswitch) {
                    # Clean up the key prefix from vmnic name
...
# Pre-gather VMkernel PortGroup names to exclude from Phase 1 replication
$clusterVmkPgs = @()
foreach ($h in $hosts) {
            ...
            if ($attachedNics -or $attachedVmks) {
                ...
                if ($repPg -eq 'y') {
                    ...
                    foreach ($dpg in $vmOnlyPgs) {
                        ...
                        # Apply the -vss suffix to Virtual Machine portgroups
...
# ==============================================================================
# PHASE 2: PHYSICAL NIC REMOVAL VIA API
# ==============================================================================
Write-Log "`n===========================================================" "White"
...
foreach ($vds in $vdsList) {
    ...
    $removeNic = Read-YesNo -PromptMessage "`nDo you want to safely remove a vmnic from '$($vds.Name)'? (y/n)"
    
    if ($removeNic -eq 'y') {
        $targetNic = Read-NicName -ValidNics $validNicOptions
        $removeAllHosts = Read-YesNo -PromptMessage "`nDo you want to remove $targetNic from ALL hosts simultaneously? (y/n - 'n' will prompt per host)"
        
        Write-Log "`n>>> Processing detachment of $targetNic for vDS '$($vds.Name)'..." "Cyan"
        $successfullyFreed = $false

        foreach ($hName in $hostNicMap.Keys) {
            if ($hostNicMap[$hName] -contains $targetNic) {
                if ($removeAllHosts -eq 'n') {
                    $removeThisHost = Read-YesNo -PromptMessage "   -> [HOST: $hName] Remove $targetNic? (y/n)"
                ...
                $tgthost.Backing.PnicSpec = @($currentBacking.PnicSpec | Where-Object { $_.PnicDevice -ne $targetNic })
                ...
                try {
                    ...
                    if ($task.Info.State -eq "success") {
                        Write-Log "      [SUCCESS] Removed $targetNic." "Green"
        ...
        if ($successfullyFreed -and $targetNic -notin $freedNicsList) {
            $freedNicsList += $targetNic
        }
    }
}
        ...
        foreach ($h in $hosts) {
            ...
            foreach ($vmk in $orderedVmks) {
                ...
                if ($moveThis -eq 'y') {
                    ...
                    # Auto-create PortGroup if it doesn't exist
                    if (-not $targetPgObj) {
...
foreach ($vds in $vdsList) {
    ...
    $validNicOptions = $allUniqueNics | Select-Object -Unique | ForEach-Object { $_.ToLower() }
    $removeNic = Read-YesNo -PromptMessage "`nDo you want to safely remove a leftover vmnic from '$($vds.Name)'? (y/n)"
    
    if ($removeNic -eq 'y') {
        $targetNic = Read-NicName -ValidNics $validNicOptions
        $removeAllHosts = Read-YesNo -PromptMessage "`nDo you want to remove $targetNic from ALL hosts simultaneously? (y/n - 'n' will prompt per host)"
        
        Write-Log "`n>>> Processing final detachment of $targetNic for vDS '$($vds.Name)'..." "Cyan"
        $successfullyFreed = $false
        ...
        if ($successfullyFreed) {
            $attachNic = Read-YesNo -PromptMessage "`nDo you want to assign the newly freed '$targetNic' as an uplink to a Standard Switch now? (y/n)"
            ...
            if ($attachNic -eq 'y') {
                ...
                if ($uniqueVss.Count -eq 0) {
                    ...
                    Write-Log "`n>>> Assigning $targetNic to $targetVssName..." "Cyan"
                    foreach ($h in $hosts) {
                        $vssObj = Get-VirtualSwitch -VMHost $h -Name $targetVssName -ErrorAction SilentlyContinue
                        $freePnicObj = Get-VMHostNetworkAdapter -VMHost $h -Physical -Name $targetNic -ErrorAction SilentlyContinue
                        
                        if ($vssObj -and $freePnicObj) {
                            try {
                                $existingUplinks = Get-VirtualSwitch -VMHost $h -Name $targetVssName | Get-VMHostNetworkAdapter -Physical -ErrorAction SilentlyContinue
                                if ($existingUplinks.Name -contains $targetNic) {
                                    Write-Log "   -> [HOST: $($h.Name)] $targetNic is already attached to $targetVssName. Skipping." "DarkGray"
                                } else {
                                    Add-VirtualSwitchPhysicalNetworkAdapter -VirtualSwitch $vssObj -VMHostPhysicalNic $freePnicObj -Confirm:$false | Out-Null
                                    Write-Log "   -> [HOST: $($h.Name)] [SUCCESS] Attached $targetNic to $targetVssName." "Green"
                                }
                            } catch {
                                Write-Log "   -> [HOST: $($h.Name)] [FAILED] $($_.Exception.Message)" "Red"
                            }
