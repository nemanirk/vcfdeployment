...
elseif ($MenuChoice -eq "1") {
    # --- MANUAL INPUT LOGIC FOR OPTION 1 ---
...
# --- [TEP Manual Input - Shared by both 1 and 2] ---
...
# --- [NEW VALIDATION] Check for sufficient Free IPs ---
...
# --- [3] Connect and Initial MTU Check (9000) ---
...
foreach ($HName in $Hosts) {
    try {
        ...
        if ($firstHost) {
            Write-Host "`n[FATAL ERROR] Failed to connect to the first host ($HName). Incorrect password or connectivity issue." -ForegroundColor Red
            exit
        } else {
            Write-Warning "Could not connect to $HName" 
        }
    }
}
...
# Optimized Ring Ping for Management
for ($i = 0; $i -lt $Hosts.Count; $i++) {
    $Source = $Hosts[$i]
    $DestIndex = ($i + 1) % $Hosts.Count
    $DestIP = $MgmtIpMap[$Hosts[$DestIndex]]
...
foreach ($Net in $NetworkConfigs) {
    ...
    for ($i=0; $i -lt $Hosts.Count; $i++) {
        ...
        if ($conn) {
            try {
                ...
                $newPg = New-VirtualPortGroup -VirtualSwitch $vSwitch -Name $DummyPGName -VLanID $Net.VLAN | Out-Null
                
                # UPDATE MESSAGE
                Write-Host "Configuring $DummyPGName on $ESXi [VLAN: $($Net.VLAN) | IP: $TargetIP]..." -ForegroundColor Yellow
                $newPg = Get-VirtualPortGroup -VMHost $vmhost -Name $DummyPGName
                $newPg | Get-NicTeamingPolicy | Set-NicTeamingPolicy -LoadBalancingPolicy ExplicitFailover -MakeNicActive "vmnic0" -MakeNicStandby "vmnic1" -Confirm:$false | Out-Null
    ...
    function Run-PingTestInternal {
        param($Active, $Standby, $NetObj, $TargetMap)
        Write-Host "`n --- TEST: ${Active} ACTIVE | Network: $($NetObj.Name) ---" -ForegroundColor Cyan
        foreach ($h in $Hosts) { 
            if ($ConnectionMap[$h]) {
                Get-VirtualPortGroup -VMHost (Get-VMHost -Server $ConnectionMap[$h]) -Name $DummyPGName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicActive $Active -MakeNicStandby $Standby -Confirm:$false | Out-Null
            }
        }
        ...
        for ($i = 0; $i -lt $Hosts.Count; $i++) {
            ...
            if ($ConnectionMap[$Source]) {
                ...
                if ($NetObj.Gateway) {
                    ...
                    $TestResults.Add([PSCustomObject]@{ Network=$NetObj.Name; VLAN=$NetObj.VLAN; Source=$Source; DestinationType="Gateway"; DestinationIP=$NetObj.Gateway; Uplink=$Active; Status=$gStatus })
    ...
    Run-PingTestInternal -Active "vmnic0" -Standby "vmnic1" -NetObj $Net -TargetMap $HostNetworkMap
    Run-PingTestInternal -Active "vmnic1" -Standby "vmnic0" -NetObj $Net -TargetMap $HostNetworkMap
    ...
    try {
        ...
        if ($conn) {
            ...
            if ($sshStatus.Running -eq $false) {
                Write-Host "Starting SSH service on ${ESXi}..." -ForegroundColor Yellow
                Start-VMHostService -HostService $sshStatus -Confirm:$false | Out-Null
            }
        ...
        Write-Warning "Cleanup failed for $ESXi" 
    }
}
