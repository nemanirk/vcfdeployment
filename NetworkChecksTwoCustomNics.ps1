...
# - Provides options to configure network inputs manually or via SDDC Network Pools.
# - Prompts to verify if the deployment uses vSAN storage (skips vSAN inputs if no).
# - Dynamically adjusts gateway ping prompts based on vSAN selection.
# - IMPORTANT: This script is strictly designed for hosts using exactly TWO vmnics for testing.
#---------------------------------------------------------

#-----------Network Validation Report (Master Optimized)-------------------
# 1. Sets vSwitch0 MTU to 9000 (if not already set).
# 2. Dynamically detects vmk0 NIC and prompts for standby NIC.
# 3. Adds standby NIC to vSwitch0 and tests Mgmt redundancy.
# 4. Pings in a "Ring Topology" (1 peer each) for speed.
# 5. Cleans up temp artifacts and resets vSwitch0 MTU to 1500
# 6. Ensures SSH is Running at the end of the script.
# 7. Calculates and displays total execution time.
#---------------------------------------------------------
...
# --- [Gateway Ping Choice] ---
if ($NetworkConfigs.Name -contains "vSAN") {
...
# --- [3] Connect, Detect NICs, and Initial MTU Check ---
...
foreach ($HName in $Hosts) {
    try {
        ...
        if ($firstHost) {
            $PhysicalNics = (Get-VMHostNetworkAdapter -VMHost $vmhost -Physical).Name
            $mgmtPGName = (Get-VMHostNetworkAdapter -VMHost $vmhost -Name "vmk0").PortGroupName
            
            # Determine which NIC is actively backing vmk0
            $mgmtTeaming = Get-VirtualPortGroup -VMHost $vmhost -Name $mgmtPGName | Get-NicTeamingPolicy
            $ActiveNic = $mgmtTeaming.ActiveNic | Select-Object -First 1
            if (-not $ActiveNic) { $ActiveNic = $vSwitch.Nic | Select-Object -First 1 } # Fallback to vSwitch config
            ...
            $AvailableNics = $PhysicalNics | Where-Object { $_ -ne $ActiveNic }

            Write-Host "`n>>> HARDWARE NETWORK DETECTION <<<" -ForegroundColor Cyan -BackgroundColor DarkBlue
            Write-Host "Detected Active NIC for vmk0 (Management): " -NoNewline; Write-Host $ActiveNic -ForegroundColor Green
            Write-Host "Other available physical NICs on host: " -NoNewline; Write-Host ($AvailableNics -join ", ") -ForegroundColor Yellow

            $validInput = $false
            while (-not $validInput) {
                $StandbyNic = Read-Host "`nEnter the second vmnic to use for testing (e.g., vmnic1)"
                if ($StandbyNic -eq $ActiveNic) {
                    Write-Host "[ERROR] You entered the NIC currently used by vmk0 ($ActiveNic). Please select a different one." -ForegroundColor Red
                } elseif ($AvailableNics -contains $StandbyNic) {
                    $validInput = $true
                    Write-Host "Using $ActiveNic and $StandbyNic for validation testing.`n" -ForegroundColor Green
                } else {
                    Write-Host "[ERROR] '$StandbyNic' is invalid. Please enter one of the available NICs: $($AvailableNics -join ', ')" -ForegroundColor Red
                }
            }
        }
...
foreach ($HName in $Hosts) {
    ...
    if ($conn) {
        $vmhost = Get-VMHost -Server $conn
        $vSwitch = Get-VirtualSwitch -VMHost $vmhost -Name $vSwitchName
        if ($vSwitch.Nic -notcontains $StandbyNic) {
            $StandbyNicObj = Get-VMHostNetworkAdapter -VMHost $vmhost -Physical -Name $StandbyNic
            Add-VirtualSwitchPhysicalNetworkAdapter -VirtualSwitch $vSwitch -VMHostPhysicalNic $StandbyNicObj -Confirm:$false | Out-Null
        }
        ...
        $mgmtPG | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicActive $StandbyNic -MakeNicStandby $ActiveNic -Confirm:$false | Out-Null
    }
}
...
for ($i = 0; $i -lt $Hosts.Count; $i++) {
    ...
    if ($ConnectionMap[$Source]) {
        ...
        $TestResults.Add([PSCustomObject]@{ Network="Management"; VLAN="Mgmt"; Source=$Source; DestinationType="Peer"; DestinationIP=$DestIP; Uplink=$StandbyNic; Status=$status })
        Write-Host "MGMT: ${Source} -> ${DestIP} via ${StandbyNic}: ${status}"
    }
}

Write-Host "Restoring Mgmt to $ActiveNic Active..." -ForegroundColor Yellow
foreach ($HName in $Hosts) {
    $conn = $ConnectionMap[$HName]
    if ($conn) {
        $vmhost = Get-VMHost -Server $conn
        $mgmtPGName = (Get-VMHostNetworkAdapter -VMHost $vmhost -Name "vmk0").PortGroupName
        Get-VirtualPortGroup -VMHost $vmhost -Name $mgmtPGName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicActive $ActiveNic -MakeNicStandby $StandbyNic -Confirm:$false | Out-Null
    }
}
...
foreach ($Net in $NetworkConfigs) {
    ...
    for ($i=0; $i -lt $Hosts.Count; $i++) {
        ...
        if ($conn) {
            try {
                ...
                Get-VirtualPortGroup -VMHost $vmhost -Name $DummyPGName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -LoadBalancingPolicy ExplicitFailover -MakeNicActive $ActiveNic -MakeNicStandby $StandbyNic -Confirm:$false | Out-Null
    ...
    Run-PingTestInternal -Active $ActiveNic -Standby $StandbyNic -NetObj $Net -TargetMap $HostNetworkMap -GwChoice $PingGwChoice
    Run-PingTestInternal -Active $StandbyNic -Standby $ActiveNic -NetObj $Net -TargetMap $HostNetworkMap -GwChoice $PingGwChoice
...
foreach ($ESXi in $Hosts) {
    try {
        ...
        if ($conn) {
            ...
            Write-Host "vSwitch0    $currentMtu       1500" -ForegroundColor Gray
            ...
            $mgmtPGName = (Get-VMHostNetworkAdapter -VMHost $vmhost -Name "vmk0").PortGroupName
            Get-VirtualPortGroup -VMHost $vmhost -Name $mgmtPGName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicActive $ActiveNic -Confirm:$false | Out-Null
            
            if ($vSwitch.Nic -contains $StandbyNic) { 
                Write-Host "Removing $StandbyNic from vSwitch0 on $ESXi..." -ForegroundColor Gray
                $vmhost | Get-VMHostNetworkAdapter -Physical -Name $StandbyNic | Remove-VirtualSwitchPhysicalNetworkAdapter -Confirm:$false | Out-Null 
            }
