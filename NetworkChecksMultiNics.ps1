#================================================================================
# VCF NETWORK PRE-CHECK UTILITY (Master Optimized v3.7)
#================================================================================
# Functionality:
#   Automates the validation of physical and logical network readiness for VMware
#   Cloud Foundation (VCF) ESXi hosts prior to bringing up a Management Domain
#   or adding hosts to workload domains. It supports 2-NIC and multi-NIC profiles.
#   The script validates MTU sizes, NIC redundancy, and performs peer-to-peer 
#   as well as gateway reachability checks for Management, vMotion, vSAN, and TEPs.
#
# Sequence of Steps Executed:
#   1. [Menu & Inputs]: Prompts user for deployment type (Manual vs SDDC Pool),
#      vSAN usage, gateway ping preferences, and network configurations.
#   2. [Authentication]: Connects to SDDC Manager (if pool selected) and ESXi hosts.
#   3. [Dynamic NIC Detection]: Automatically detects the active physical NIC backing
#      vmk0 (Management) and maps remaining available NICs for testing.
#   4. [Host Setup]: Sets vSwitch0 MTU to 9000 and adds necessary uplinks.
#   5. [Management Redundancy Test]: Tests Mgmt network failover by failing over to 
#      the designated standby NIC and pinging the adjacent host's Management IP.
#   6. [Iterative Traffic Testing]: For each configured network (vMotion, vSAN, TEP):
#       a. Creates a temporary port group (TEMP-PRECHECK-PG) and VMkernel adapter.
#       b. Applies Explicit Failover teaming per required active uplink.
#       c. Executes pings (Jumbo frames) to peer hosts, gateways, and Edge nodes.
#       d. Destroys the temporary VMkernel adapter and port group.
#   7. [Final Cleanup]: Reverts vSwitch0 MTU to 1500, removes testing NICs from the 
#      vSwitch, and properly disconnects host sessions.
#   8. [Reporting]: Exports a detailed CSV report containing all test results.
#================================================================================
...
$NicMap = @{}
# Note: Mgmt NIC logic has been moved to Section [3] where we dynamically detect it
...
if ($MenuChoice -eq "2") {
    ...
    try {
        ...
        foreach ($net in $NetResponse.elements) {
            # Temporary placeholder for NICs until Section 3 populates $NicMap
            $NetworkConfigs += [PSCustomObject]@{ Name=$net.type; VLAN=$net.vlanId; Mask=$net.mask; Gateway=$net.gateway; FreeIps=$net.freeIps; Nics=$null }
        }
    } catch {
...
elseif ($MenuChoice -eq "1") {
    ...
    $isVsan = Read-Host "Is this a VSAN storage deployment? (y/n)"
    $NetTypes = @("vMotion")
    if ($isVsan -match '^[Yy]') { $NetTypes += "vSAN" }

    foreach ($type in $NetTypes) {
        ...
        # Temporary placeholder for NICs until Section 3 populates $NicMap
        $NetworkConfigs += [PSCustomObject]@{ Name=$type; VLAN=$vlan; Mask=$mask; Gateway=$gw; FreeIps=$ips; Nics=$null }
    }
}
...
# Temporary placeholder for NICs until Section 3 populates $NicMap
$NetworkConfigs += [PSCustomObject]@{ Name="NSX_TEP"; VLAN=$tVlan; Mask=$tMask; Gateway=$tGw; FreeIps=$tIps; Nics=$null }

# --- [Gateway Ping Choice] ---
$hasVsan = ($NetworkConfigs.Name -contains "vSAN")
if ($hasVsan) {
    $PingGwChoice = Read-Host "`nDo you want to ping vMotion and vSAN gateways? (y/n)"
} else {
    $PingGwChoice = Read-Host "`nDo you want to ping vMotion gateway? (y/n)"
}
...
# --- [3] Connect & Dynamic NIC Validation ---
...
foreach ($HName in $Hosts) {
    try {
        ...
        # Dynamic NIC Detection on the first host
        if ($firstHost) {
            $mgmtPGName = (Get-VMHostNetworkAdapter -VMHost $vmhost -Name "vmk0").PortGroupName
            $mgmtTeaming = Get-VirtualPortGroup -VMHost $vmhost -Name $mgmtPGName | Get-NicTeamingPolicy
            $ActiveMgmtNic = $mgmtTeaming.ActiveNic | Select-Object -First 1
            if (-not $ActiveMgmtNic) { $ActiveMgmtNic = $vSwitch.Nic | Select-Object -First 1 }
            
            if (-not $ActiveMgmtNic) {
                Write-Host "[FATAL ERROR] Could not detect an active uplink for vmk0 on $HName" -ForegroundColor Red
                exit
            }

            $AvailableNicsWithoutActive = $AvailableNicNames | Where-Object { $_ -ne $ActiveMgmtNic }
            
            Write-Host "`n>>> HARDWARE NETWORK DETECTION <<<" -ForegroundColor Cyan -BackgroundColor DarkBlue
            Write-Host "Detected Active NIC for vmk0 (Management): " -NoNewline; Write-Host $ActiveMgmtNic -ForegroundColor Green
            Write-Host "Other available physical NICs on host: " -NoNewline; Write-Host ($AvailableNicsWithoutActive -join ", ") -ForegroundColor Yellow

            # Populate NicMap dynamically based on choice
            if ($NicChoice -eq "2") {
                $validMgmtInput = $false
                while (-not $validMgmtInput) {
                    $StandbyMgmtNic = Read-Host "Additional nic for Mgmt Network (e.g., vmnic2)"
                    if ($AvailableNicsWithoutActive -contains $StandbyMgmtNic) { $validMgmtInput = $true }
                    else { Write-Host "[ERROR] Invalid NIC. Available: $($AvailableNicsWithoutActive -join ', ')" -ForegroundColor Red }
                }
                $NicMap["Mgmt"] = $StandbyMgmtNic
                
                $NicMap["vMotion"] = (Read-Host "NICS for VMotion Network (e.g., vmnic2,vmnic3)").Split(',').Trim()
                if ($hasVsan) { $NicMap["vSAN"] = (Read-Host "NICS for VSAN Network (e.g., vmnic4,vmnic5)").Split(',').Trim() }
                $NicMap["NSX_TEP"] = (Read-Host "NICS for NSX_TEP Network (e.g., vmnic6,vmnic7)").Split(',').Trim()
            } else {
                $validMgmtInput = $false
                while (-not $validMgmtInput) {
                    $StandbyMgmtNic = Read-Host "`nEnter the second vmnic to use for testing (e.g., vmnic1)"
                    if ($AvailableNicsWithoutActive -contains $StandbyMgmtNic) { $validMgmtInput = $true }
                    else { Write-Host "[ERROR] Invalid NIC. Available: $($AvailableNicsWithoutActive -join ', ')" -ForegroundColor Red }
                }
                $NicMap["Mgmt"]    = $StandbyMgmtNic
                $NicMap["vMotion"] = @($ActiveMgmtNic, $StandbyMgmtNic)
                if ($hasVsan) { $NicMap["vSAN"] = @($ActiveMgmtNic, $StandbyMgmtNic) }
                $NicMap["NSX_TEP"] = @($ActiveMgmtNic, $StandbyMgmtNic)
            }

            $AllRequiredNics = ($NicMap.Values | ForEach-Object { $_ }) | Select-Object -Unique | Where-Object { $_ -ne "" }
            
            # Map the NICs back to the NetworkConfigs objects we created earlier
            foreach ($Net in $NetworkConfigs) {
                $Net.Nics = $NicMap[$Net.Name]
            }
        }

        foreach ($ReqNic in $AllRequiredNics) {
            if ($AvailableNicNames -notcontains $ReqNic -and $ReqNic -ne $ActiveMgmtNic) {
        ...
        foreach ($nicName in ($AllRequiredNics | Where-Object { $_ -ne $ActiveMgmtNic })) {
...
foreach ($HName in $Hosts) {
    if ($ConnectionMap[$HName]) {
        ...
        Get-VirtualPortGroup -VMHost $vmhost -Name $pgName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicActive $targetMgmtNic -MakeNicStandby $ActiveMgmtNic -Confirm:$false | Out-Null
    }
}
...
Write-Host "Restoring Mgmt to $ActiveMgmtNic Active..." -ForegroundColor Yellow
foreach ($HName in $Hosts) {
    if ($ConnectionMap[$HName]) {
        $vmhost = Get-VMHost -Server $ConnectionMap[$HName]
        $pgName = (Get-VMHostNetworkAdapter -VMHost $vmhost -Name "vmk0").PortGroupName
        Get-VirtualPortGroup -VMHost $vmhost -Name $pgName | Get-NicTeamingPolicy | Set-NicTeamingPolicy -MakeNicActive $ActiveMgmtNic -MakeNicStandby $targetMgmtNic -Confirm:$false | Out-Null
    }
}
...
foreach ($ESXi in $Hosts) {
    if ($ConnectionMap[$ESXi]) {
        ...
        foreach ($nic in ($AllRequiredNics | Where-Object { $_ -ne $ActiveMgmtNic })) {
