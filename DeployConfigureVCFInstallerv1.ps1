...
# --- [1] Select Input Method (WITH VALIDATION LOOP) ---
$InputMethod = ""
do {
    $InputMethod = Read-Host "Use Config File (F) or Manual Entry (M)?"
    if ($InputMethod -notmatch '^[FfMm]$') {
        Write-Host "[INVALID INPUT] Please enter F or M." -ForegroundColor Red
    }
} while ($InputMethod -notmatch '^[FfMm]$')
...
# --- INITIALIZATION DELAY AFTER DEPOT CONFIG ---
Write-Host "Sleeping for 5 mins. Initializing binary database, please wait..." -ForegroundColor Yellow
...
if ($null -eq $BundlesToDownload -or ($BundlesToDownload | Measure-Object).Count -eq 0) {
    ...
    # --- [8] Phase 6: Refreshing Monitoring Dashboard (ROBUST VALIDATION) ---
    ...
    while (-not $AllFinished) {
        ...
        if ((Get-Date) -gt $MonitorStartTime.AddMinutes($MaxMinutes)) {
            Write-Host "`n[TIMEOUT] 2-hour limit reached. Sounding alert." -ForegroundColor Red
        ...
        try {
            ...
            Write-Host " Elapsed: $([math]::Round(((Get-Date) - $MonitorStartTime).TotalMinutes, 1)) / $MaxMinutes min" -ForegroundColor Gray
            Write-Host "==========================================================" -ForegroundColor Cyan
            
            if ($null -eq $MyBundlesStatus -or ($MyBundlesStatus | Measure-Object).Count -eq 0) {
                Write-Host "Waiting for bundle statuses to appear in database..." -ForegroundColor Yellow
            } else {
                $MyBundlesStatus | Select-Object componentType, version, downloadStatus | Format-Table -AutoSize

                # --- EXCLUSION-BASED VALIDATION LOGIC ---
                # Check for any state that is NOT terminal (SUCCESSFUL/FAILED)
                $TerminalStates = @("SUCCESSFUL", "SUCCESS", "FAILED")
                $ActiveItems = $MyBundlesStatus | Where-Object { $_.downloadStatus -notin $TerminalStates }
                
                # Only exit if we have bundle data AND no active items remain
                if (($MyBundlesStatus | Measure-Object).Count -gt 0 -and ($null -eq $ActiveItems -or ($ActiveItems | Measure-Object).Count -eq 0)) {
                    $AllFinished = $true
                    Write-Host "`n[COMPLETE] All triggered downloads have finished." -ForegroundColor Green
                    [System.Console]::Beep(1000, 500) # Success Alert
                } else {
